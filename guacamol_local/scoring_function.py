...
from guacamol_local.utils.chemistry import smiles_to_rdkit_mol
from guacamol_local.score_modifier import ScoreModifier, LinearModifier
from guacamol_local.sa_modifier import SAModifier
from guacamol_local.utils.math import geometric_mean
...
class BatchScoringFunction(ScoringFunction):
    ...
    def score_list(self, smiles_list: List[str]) -> List[float]:
        ...
        scores = [self.corrupt_score if raw_scores[i] is None
                  else self.modify_score(raw_scores[i])
...
class ScoringFunctionSAWrapper(ScoringFunction):
    """
    Wraps a scoring function to take synthesizability into account
    """

    def __init__(self, scoring_function: ScoringFunction, sa_modifier: SAModifier = None) -> None:
        super().__init__()
        self.scoring_function = scoring_function
        self.sa_modifier = LinearModifier() if sa_modifier is None else sa_modifier
    ...
    # @sa_modifier.setter
    # def sa_modifier(self, modifier: Optional[SAModifier]):
    #     self._sa_modifier = LinearModifier() if modifier is None else modifier
